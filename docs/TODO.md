...
closed 2026-08-26 (measured, no code change — its own document-frequency hypothesis did
not survive the measurement); B-039 added the same day, split out of what that
measurement actually found. Anchors were verified against the tree at that date; re-grep
...
- **PLANNED** — workable, has a full six-field section in this file. **None open right
  ...
  opened B-036 (closed 2026-08-26, no code change) and B-032's opened B-037; B-036's own
  closure opened B-039 in its place. None of B-037/B-038/B-039 is PLANNED — all three are
  NO STEPS by their own argument, each naming a measurement to run before any design is
  chosen, not a fix to apply.
...
| B-039 | `Rank`'s `TopN=10` window saturates on most adjacent-topic queries | NO STEPS — measure first | [below](#no-steps) |
...
## B-039 — `recall.Rank`'s `TopN=10` window saturates on most adjacent-topic queries

**Closed 2026-08-26: B-036, this section's previous occupant, measured its own unblock
condition and the hypothesis it named (a small number of document-frequent notes forcing
the ten-neighbour cap) did not survive the measurement — see BACKLOG.md's B-036 closing
section for the numbers. The same measurement found a different, real mechanism, which is
this item.**

`cmd/forge/neighbour_frequency_test.go`'s harness (built to close B-036) also counts how
many of its 15 labelled queries hit `recall.Rank`'s `TopN=10` truncation before
`Thresholds.Neighbours` ever filters by the floor: **14 of 15**. `calibration.golden`
shows the same shape at a smaller sample (3 of 9). Nearly every adjacent-topic query
saturates the window — that could be the corpus's real topical density working as
intended, or a window sized without evidence either way; nothing today distinguishes
the two.

**Unblock condition:** widen the *internal* ranking window only (a second, larger
constant feeding `recall.Rank`'s scoring pass — not the emitted `TopN`, which is
`recall-spec.md` §4's output contract) and re-run `TestNeighbourFloorSweep` and
`TestNeighbourDocumentFrequency` to see whether candidates 11-15 cluster near candidate
10's score (saturation, arguably fine as-is) or fall off sharply (real evidence `TopN=10`
is already the right cut). Design after reading it — same discipline B-036 named and
this time actually ran.
