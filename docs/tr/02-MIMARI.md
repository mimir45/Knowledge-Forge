...
## 16. Faz geçmişi (mimarinin nasıl büyüdüğü)
...
| 3 | `pkg/config` dört katmanlı zincir; `forge config/init` |
| 3b | `pkg/engine` dört tier; `forge engine select/run/record`; SQLite bütçe; `cost.md` |
| 4 | `pkg/qualitygate` yedi kapı; `forge gate`, `forge verify-code`; product agent'lar |
| 5 | Claude Code hook'ları: `session-context`, `intent`, `session-capture`, `cache-source`; `forge stats`; drift git hook'ları |
| 5b | `pkg/sentinel`; `forge logback` |
| 6 | Plugin manifest; `pkg/scrub` + `forge scrub`; `examples/vault/`; ADR'ler; README/LICENSE/CHANGELOG/CONTRIBUTING; `evals/`; `.golangci.yml` |
| **6b** | **Sıradaki:** `/forge-export-dataset`, `/forge-dataset-stats` |
