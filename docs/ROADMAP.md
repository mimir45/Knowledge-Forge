...
> [`KNOWLEDGE-FORGE-B2B.md`](./KNOWLEDGE-FORGE-B2B.md) ·
...
pipeline stage. v2 also lays the groundwork for a B2B "no context loss"
product (Slack/GitHub ingestion → org wiki → MCP server) without building it
yet.

**Status:** design complete across 5 docs. No code, no vault, no skill exists
in this repo yet — `go.mod` is the only artifact. Phase 0 has not run.
...
| 7 | **Polish on real usage** | Prove the thesis with real numbers | Run it daily for ~1 month, `docs/RESULTS.md` (hit-rate trend, dedup rate, drift events, time saved), launch post drafted, first LoRA experiment if D1 ≥ 300 pairs | `/forge-stats` shows a real, citable hit-rate number | ongoing (~1 month) |

**Total: ~4-5 weeks of focused evenings** to a public v2.0 (Go ramp + release
tooling adds ~1-1.5 weeks vs. the original Python estimate).
...
- B2B (`KNOWLEDGE-FORGE-B2B.md`) is explicitly **out of scope until Phase 7's
  gate**: "OSS v2.0 shipped, 30 days of real usage, ≥3 outside users
  reporting value." Don't start B1 before that.
