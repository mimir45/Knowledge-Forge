...
// Result decides the verdict for a ranked candidate list and packages both.
//
// Neighbours are populated on CREATE only. The 0.30–0.55 band exists to answer "what
...
func (t Thresholds) Result(q Query, cands []Candidate) Result {
	r := Result{Question: q.Question, Candidates: cands, Neighbours: []Candidate{}}
	if r.Candidates == nil {
	...
	if r.Verdict = t.Decide(top); r.Verdict == Create {
		r.Neighbours = append(r.Neighbours, t.Neighbours(r.Candidates)...)
	}
	return r
}
