...
// every capture is appended as its own line — no Key(), no dedup. D4Tag is spelled
// correctly ("d4"), unlike D2Tag's shipped mismatch against the packaged
// dataset.capture list (see BACKLOG) — so D4 fires under that config where D2 silently
// does not. That asymmetry is intentional, not a bug to fix here.
const (
