...
//
// Its input does not exist yet, and the report says so rather than inventing a substitute.
// The ask log is written by the D3 capture hook, which is installed and live but has
// recorded nothing: every note in the vault is `origin: import`, so there are no asks to
// count. Filling this page from some other signal — unlinked mentions, say — would produce
// a plausible list of topics that nobody actually asked about, which is worse than an empty
// page because it cannot be told apart from a real one.
